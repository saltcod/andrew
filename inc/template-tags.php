...
if ( ! function_exists( 'waterstreet_content_nav' ) ):
/**
 * Display navigation to next/previous pages when applicable
 *
 * @since waterstreet 1.0
 */
function waterstreet_content_nav( $nav_id ) {
...
false, '', true );
	...
	<nav role="navigation" id="<?php echo $nav_id; ?>" class="<?php echo $nav_class; ?>">
		<h1 class="assistive-text"><?php _e( 'Post navigation', 'waterstreet' ); ?></h1>
...
endif; // waterstreet_content_nav

if ( ! function_exists( 'waterstreet_comment' ) ) :
/**
 ...
 * @since waterstreet 1.0
 */
function waterstreet_comment( $comment, $args, $depth ) {
...
endif; // ends check for waterstreet_comment()

if ( ! function_exists( 'waterstreet_posted_on' ) ) :
/**
...
fn n" href="%5$s" title="%6$s" rel="author">%7$s</a></span></span>', 'waterstreet' ),
		...
		esc_attr( sprintf( __( 'View all posts by %s', 'waterstreet' ), get_the_author() ) ),
 ...
 * @since waterstreet 1.0
 */
function waterstreet_categorized_blog() {
	...
	if ( '1' != $all_the_cool_cats ) {
		// This blog has more than 1 category so waterstreet_categorized_blog should return true
		return true;
	} else {
		// This blog has only 1 category so waterstreet_categorized_blog should return false
		return false;
	}
}

/**
 * Flush out the transients used in waterstreet_categorized_blog
 *
 * @since waterstreet 1.0
 */
function waterstreet_category_transient_flusher() {
	// Like, beat it. Dig?
	delete_transient( 'all_the_cool_cats' );
}
add_action( 'edit_category', 'waterstreet_category_transient_flusher' );
add_action( 'save_post', 'waterstreet_category_transient_flusher' );
